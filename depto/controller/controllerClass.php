...
class controllerClass {
  ...
  public function create() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      $anwr = modelClass::newDepto($_POST['txtCod'], $_POST['txtNom']);
      if ($anwr === true) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' and isset($_GET['id']) and is_numeric($_GET['id'])) {
      ...
      if (is_array($certificate)) {
        if (count($certificate) > 0) {
          ...
          if (is_array($data)) {
            if (count($data) > 0) {
              $args['txtCod'] = $data[0]['cod_depto'];
              $args['txtNom'] = $data[0]['nom_depto'];
            }
          } else {
            $args['error'] = $data;
            viewClass::renderHTML('error.php', $args);
      ...
      $data['nom_depto'] = $_POST['txtNom'];

      $rsp = modelClass::updateDepto($_GET['id'], $data);
      if ($rsp === true) {
  ...
  public function delete() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' and isset($_GET['id']) and is_numeric($_GET['id'])) {
      ...
      $rsp = modelClass::deleteDepto($_GET['id']);
      if ($rsp === true) {
