...
class controllerClass {
  ...
  public function create() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      $anwr = modelClass::newCredencial($_POST['txtId'], $_POST['txtuser'], $_POST['txtcredencial']);
      if ($anwr === true) {
  ...
  public function update() {
    if ($_SERVER['REQUEST_METHOD'] === 'GET' and isset($_GET['id']) and is_numeric($_GET['id'])) {
        ...
        if (count($certificate) > 0) {
          ...
          if (is_array($data)) {
            if (count($data) > 0) {
              ...
              $args['txtuser'] = $data[0]['usuario_id'];
              $args['txtcredencial'] = $data[0]['credencial_id'];
            }
          } else {
            $args['error'] = $data;
            viewClass::renderHTML('error.php', $args);
      ...
      $data['usuario_id'] = $_POST['txtuser'];
      $data['credencial_id'] = $_POST['txtcredencial'];
