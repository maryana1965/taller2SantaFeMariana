...
class controllerClass {
  ...
  public function create() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      $anwr = modelClass::newCausaDesercion($_POST['txtCod'], $_POST['txtDesc'], $_POST['txtEstado']);
      if ($anwr === true) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' and isset($_GET['id']) and is_numeric($_GET['id'])) {
      ...
      if (is_array($certificate)) {
        if (count($certificate) > 0) {
          ...
          if (is_array($data)) {
            if (count($data) > 0) {
              $args['txtCod'] = $data[0]['cod_causa'];
              $args['txtDesc'] = $data[0]['desc_causa'];
      ...
      $data['desc_causa'] = $_POST['txtDesc'];
      $data['estado'] = $_POST['txtEstado'];
      

      $rsp = modelClass::updateCausaDesercion($_GET['id'], $data);
      if ($rsp === true) {
  ...
  public function delete() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' and isset($_GET['id']) and is_numeric($_GET['id'])) {
      ...
      $rsp = modelClass::deleteCausaDesercion($_GET['id']);
      if ($rsp === true) {
